...
public class CardFieldView extends GridPane implements ViewObserver {
    ...
    //
    public void updateBackground(Player player, int index) {
        if (player.board.getPhase() == Phase.PROGRAMMING) {
            setBackground(BG_DEFAULT);
        } else {
            if (index < player.board.getStep()) {
                setBackground(BG_DONE);
            } else if (index == player.board.getStep()) {
    ...
    private class OnDragDetectedHandler implements EventHandler<MouseEvent> {
        ...
        public void handle(MouseEvent event) {
            ...
            if (target instanceof CardFieldView source) {
                CommandCardField cardField = source.field;

                if (isDraggableCardField(cardField)) {
                    initiateDrag(source, cardField);
                }
            }
            event.consume();
        }

        private boolean isDraggableCardField(CommandCardField cardField) {
            return cardField.getCard() != null &&
                    cardField.player != null &&
                    cardField.player.board.getPhase().equals(Phase.PROGRAMMING);
        }

        private void initiateDrag(CardFieldView source, CommandCardField cardField) {
